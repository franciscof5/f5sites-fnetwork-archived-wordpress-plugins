...
class RSSimpleProduct {
    ...
    public function __construct() {
        ...
        add_filter('woocommerce_get_price_html', array($this, 'display_point_price_in_simple_product'), 99, 2);
    ...
    public static function shortcode_for_point_price($product_id) {

        global $post;
        
        $checkproducttype = get_product($product_id);
        ...
        if (is_shop() || is_product() || is_page() || is_product_category()) {
            ...
            if ($checkproducttype->is_type('simple') || ($checkproducttype->is_type('subscription'))) {
                $enablerewards = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($product_id, '_rewardsystem_enable_point_price'); //product level enable

                $getpoints = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($product_id, '_rewardsystem__points');                
    ...
    public static function display_point_price_in_simple_product($price, $product) {
        global $post;

        if (get_option('rs_enable_disable_point_priceing') == '1') {
            $labelpoint = get_option('rs_label_for_point_value');
            $enabledpoints = self::shortcode_for_point_price($product->id);

            if ($enabledpoints != '') {
                return $price . $labelpoint . $enabledpoints;
            } else {
                return $price;
            }
        }else{
    ...
    public static function display_point_price_in_variable_product($price, $object) {
        ...
        if (get_option('rs_enable_disable_point_priceing') == '1') {
            ...
            $labelpoint = get_option('rs_label_for_point_value');
            $enabledpoints = RSVariableProduct::add_shortcode_for_rewardpoints_of_variation_point($object->variation_id);
                                    ...
                                    if (get_option('_rs_enable_disable_gift_icon') == '1') {
                                        if (get_option('rs_image_url_upload') != '') {
                                            if(get_option('rs_message_position_in_single_product_page_for_variable_products') == '1'){
                                                ...
                                                return "<span class='variableshopmessage'><img src=" . get_option('rs_image_url_upload') . " style='width:16px;height:16px;display:inline;' />&nbsp;" . $price.do_shortcode(get_option('rs_message_for_single_product_variation')) . "</span>";
                                            }
                                        } else {
                                            if(get_option('rs_message_position_in_single_product_page_for_variable_products') == '1'){
                                                return "<span class='variableshopmessage'>" . do_shortcode(get_option('rs_message_for_single_product_variation')) . "</span>" . $price;
                                            }else{
                                                return $price."<span class='variableshopmessage'>" . do_shortcode(get_option('rs_message_for_single_product_variation')) . "</span>" ;
                                            }
                                        }
                                    } else {                                       
                                        if(get_option('rs_message_position_in_single_product_page_for_variable_products') == '1'){
