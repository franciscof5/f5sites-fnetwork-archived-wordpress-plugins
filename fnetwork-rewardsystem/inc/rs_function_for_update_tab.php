                ...
                <script type="text/javascript">
                    ...
                    jQuery(document).ready(function () {
                        ...
                        if ((jQuery('.rs_which_point_precing_product_selection').val() === '1')) {
                            ...
                            jQuery('#rs_select_particular_categories_for_point_price').parent().parent().hide();
                        } else if (jQuery('.rs_select_particular_products_for_point_price').val() === '3') {
                            jQuery('#rs_select_particular_products_for_point_price').parent().parent().hide();
                            jQuery('#rs_select_particular_categories_for_point_price').parent().parent().hide();
                        } else {
...
jQuery('.rs_which_point_precing_product_selection').change(function () {
        ...
        if (isset($_POST['proceedanyway'])) {
            if ($_POST['proceedanyway'] == '1') {
                if ($_POST['whichproduct'] == '1') {
                    ...
                    if (is_array($_POST['selectedproducts']) && !empty($_POST['selectedproducts'])) {
                        ...
                        foreach ($_POST['selectedproducts']as $particularpost) {
                            ...
                            if ($checkprod->is_type('simple') || ($checkprod->is_type('subscription'))) {
                                if ($_POST['enabledisablepoints'] == '1') {
                                    ...
                                    //RSFunctionForSavingMetaValues::rewardsystem_update_post_meta($particularpost, 'rs_enable_disable_point_priceing', 'yes');
                                } else {
                                    RSFunctionForSavingMetaValues::rewardsystem_update_post_meta($particularpost, '_rewardsystem_enable_point_price', 'no');
                                    // RSFunctionForSavingMetaValues::rewardsystem_update_post_meta($particularpost, 'rs_enable_disable_point_priceing', 'no');
                                }
            ...
            foreach ($products as $product) {
                ...
                if ($checkproduct->is_type('simple') || ($checkproduct->is_type('subscription'))) {
                    if ($_POST['enabledisablepoints'] == '1') {
                        ...
                        //  RSFunctionForSavingMetaValues::rewardsystem_update_post_meta($product, 'rs_enable_disable_point_priceing', 'yes');
                    } else {
                        RSFunctionForSavingMetaValues::rewardsystem_update_post_meta($product, '_rewardsystem_enable_point_price', 'no');
                        //  RSFunctionForSavingMetaValues::rewardsystem_update_post_meta($product, 'rs_enable_disable_point_priceing', 'no');
                    }
    ...
    if (isset($_POST['proceedanyway'])) {
        if ($_POST['proceedanyway'] == '1') {                      
            $args = array('post_type' => 'shop_order', 'numberposts' => '-1','meta_query'=>array(array('key'=>'reward_points_awarded','compare'=>'NOT EXISTS')),'post_status' => 'published', 'fields' => 'ids', 'cache_results' => false);            
