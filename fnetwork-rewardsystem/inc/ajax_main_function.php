        ...
        /* Reward Points For Purchasing the Product */                
            foreach ($order->get_items() as $item) {
                ...
                if ($banning_type != 'earningonly' && $banning_type != 'both') {
                    ...
                    self::rs_insert_the_selected_level_in_referral_reward_points($pointsredeemed,$referreduser,$equearnamt,$equredeemamt,$checked_level_for_referral_reward_points, $productid, $variationid, $itemquantity, $orderuserid, $termid,$order_id,'','','');                                                            

                    } else {
                        $referrer_id = RSFunctionForManualReferralLink::rs_perform_manual_link_referer($order->user_id);
                        if ($referrer_id != false) {
                            $checked_level_for_referral_reward_points = RSPointExpiry::check_level_of_enable_referral_reward_point($productid, $variationid, $termid);
                            self::rs_insert_the_selected_level_in_referral_reward_points($pointsredeemed,$referrer_id,$equearnamt,$equredeemamt,$checked_level_for_referral_reward_points, $productid, $variationid, $itemquantity, $orderuserid, $termid,$order_id,'','','');                                                
                        }
