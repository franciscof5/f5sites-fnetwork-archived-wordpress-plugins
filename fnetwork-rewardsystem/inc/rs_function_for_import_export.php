        ...
        if ($export_type_selection == '1') {
            if ($export_date_selection == '1') {
                foreach (get_users() as $users) {
                    ...
                    $users_id = $users->ID;
                    $total_points = $wpdb->get_results("SELECT * FROM $table_name WHERE userid = $users_id", ARRAY_A);
                    if (!empty($total_points)) {
                        foreach ($total_points as $new_total_points) {                                                       
                            $earnpoints = $new_total_points['earnedpoints'];                            
                            $roundofftype = get_option('rs_round_off_type') == '1' ? '2' : '0';
                            $points = round($earnpoints, $roundofftype);                                                        
                            $expirydate = $new_total_points['expirydate'];
                            if ($expirydate != 999999999999) {
                                $convertexpiry = date("m/d/Y h:i:s A T", $expirydate);
                            } else {
                                $convertexpiry = '0';
                            }                            
                ...
                if (get_option('rs_selected_user_list_export') != NULL) {
                    ...
                    foreach (get_users() as $users) {
                        ...
                        if (in_array($user_id, $list_of_users_to_export_csv)) {
                            $total_points = $wpdb->get_results("SELECT * FROM $table_name WHERE userid = $user_id", ARRAY_A);
                            if (!empty($total_points)) {
                                foreach ($total_points as $new_total_points) {                                                                                                           
                                    $earnpoints = $new_total_points['earnedpoints'];                                    
                                    $roundofftype = get_option('rs_round_off_type') == '1' ? '2' : '0';
...
// do something
        ...
        if (isset($_POST['rs_export_user_points_csv'])) {
            ...
            header("Expires: 0");
            $export_csv_heading = "User Name,Points,Event,ExipryDate" . "\n";
            //echo $export_csv_heading;
            self::outputCSV($data);
            exit();
        }
        ?>
               ...
               $i = 0;
                foreach ($getvalues as $newvalues) {
                    ...
                    if (!empty($datalogin)) {
                        ...
                        $date = $newvalues[2];
                        //
                         RSPointExpiry::insert_earning_points($usedid, $earnpoint, $pointsredeemed, $date, 'IMPOVR', $order_id, $totalearnedpoints, $totalredeempoints,'');
                        ...
                        RSPointExpiry::record_the_points($usedid, $earnpoint, $pointsredeemed, $date, 'IMPOVR', $equearnamt, $equredeemamt, '0', '0', '0', '0', '', $totalpoints, '', '0');
                    }else{
            ...
            if (is_array($getvalues)) {
                foreach ($getvalues as $newvalues) {
                    ...
                    if (!empty($datalogin)) {
                        ...
                        $earnpoint = $newvalues[1];
                        $date = $newvalues[2];
                        RSPointExpiry::insert_earning_points($usedid, $earnpoint, $pointsredeemed, $date, 'IMPADD', $order_id, $totalearnedpoints, $totalredeempoints,'');
                        $equearnamt = RSPointExpiry::earning_conversion_settings($earnpoint);
                        $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($usedid);
                        RSPointExpiry::record_the_points($usedid, $earnpoint, $pointsredeemed, $date, 'IMPADD', $equearnamt, $equredeemamt, '0', '0', '0', '0', '', $totalpoints, '', '0');
                    }else{
                        $datalogin = get_user_by('email', $newvalues[0]);
                        if (!empty($datalogin)) {
                        $oldpoints = RSPointExpiry::get_sum_of_total_earned_points($datalogin->ID);
