...
class RSRegistrationPoints {
            ...
            if (is_array($fetchdata)) {
                ...
                if ($user_id) {
                    ...
                    if (RSFunctionForSavingMetaValues::rewardsystem_get_user_meta($userid, '_points_awarded') != '1') {
                        ...
                        $redeempoints = RSFunctionToApplyCoupon::update_redeem_reward_points_to_user($order_id,$user_id);
                        $pointsredeemed = RSPointExpiry::perform_calculation_with_expiry($redeempoints, $user_id);
                        
                        if($enableoption == 'yes'){
                            if($checkredeeming == false){
                                RSPointExpiry::insert_earning_points($user_id, $curregpoints,$pointsredeemed, $date,'RRP','','','','');
                                ...
                                RSPointExpiry::record_the_points($user_id, $curregpoints,$pointsredeemed,$date,'RRP',$equearnamt,$equredeemamt,$order_id,$productid,$variationid,'0','',$totalpoints,'','0'); 
                            }
                        }else{
                            RSPointExpiry::insert_earning_points($user_id, $curregpoints,$pointsredeemed,$date, 'RRP','','','','');
                            $equearnamt = RSPointExpiry::earning_conversion_settings($curregpoints);
                            ...
                            if(($restrictuserpoints != '') && ($restrictuserpoints != '0')){
                                ...
                                if($getoldpoints <= $restrictuserpoints){
                                    ...
                                    if($totalpointss <= $restrictuserpoints){
                                        ...
                                        $redeempoints = RSFunctionToApplyCoupon::update_redeem_reward_points_to_user($order_id,$refuserid);
                                        $pointsredeemed = RSPointExpiry::perform_calculation_with_expiry($redeempoints, $refuserid);
                                        RSPointExpiry::insert_earning_points($refuserid, $refregpoints,$pointsredeemed,$date, 'RRRP','','','','');
                                        ...
                                        RSPointExpiry::record_the_points($refuserid, $refregpoints,$pointsredeemed,$date,'RRRP',$equearnamt,$equredeemamt,$order_id,$productid,$variationid,$user_id,'',$totalpoints,'','0');
                                    }else{
                                        $insertpoints = $restrictuserpoints - $getoldpoints;
                                        RSPointExpiry::insert_earning_points($refuserid, $insertpoints,$pointsredeemed,$date,'MREPFU',$order_id,$totalearnedpoints,$totalredeempoints,'');
                                        $equearnamt = RSPointExpiry::earning_conversion_settings($insertpoints);
                                        $equredeemamt = RSPointExpiry::redeeming_conversion_settings($pointsredeemed);
                                        $productid = $item['product_id'];
                                        $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($refuserid);
                                        RSPointExpiry::record_the_points($refuserid, $insertpoints,$pointsredeemed,$date,'MREPFU',$equearnamt,$equredeemamt,$order_id,$productid,'0',$user_id,'',$totalpoints,'','0');                       
                                    }
                                }else{
    ...
    public static function rs_add_regpoints_to_user_instantly($user_id) {
        ...
        $redeempoints = RSFunctionToApplyCoupon::update_redeem_reward_points_to_user($order_id,$user_id);
        $pointsredeemed = RSPointExpiry::perform_calculation_with_expiry($redeempoints, $user_id);
        RSPointExpiry::insert_earning_points($user_id, $currentregistrationpoints,$pointsredeemed,$date, 'RRP','','','','');
        $equearnamt = RSPointExpiry::earning_conversion_settings($currentregistrationpoints);
        $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($user_id);
        RSPointExpiry::record_the_points($user_id, $currentregistrationpoints,$pointsredeemed,$date,'RRP',$equearnamt,'0','0','0','0','0','',$totalpoints,'','0');
        add_user_meta($user_id, '_points_awarded', '1');
    }
                        ...
                        if(($restrictuserpoints != '') && ($restrictuserpoints != '0')){
                            ...
                            if($getoldpoints <= $restrictuserpoints){
                                ...
                                if($totalpointss <= $restrictuserpoints){
                                    ...
                                    RSPointExpiry::insert_earning_points($refuserid, $referral_registration_points,$pointsredeemed,$date, 'RRRP','0',$totalearnedpoints,'0','');                    
                                    $equearnamt = RSPointExpiry::earning_conversion_settings($referral_registration_points);
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($refuserid);
                                    RSPointExpiry::record_the_points($refuserid, $referral_registration_points,$pointsredeemed,$date,'RRRP',$equearnamt,'0','0','0','0',$user_id,'',$totalpoints,'','0');
                                    $previouslog = get_option('rs_referral_log');                    
                                    RS_Referral_Log::main_referral_log_function($refuserid, $user_id, $referral_registration_points, array_filter((array) $previouslog));
                                    update_user_meta($user_id, '_rs_i_referred_by', $refuserid);    
                                }else{
                                    ...
                                    $equredeemamt = RSPointExpiry::redeeming_conversion_settings($pointsredeemed);                                   
                                    $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($refuserid);
                                    RSPointExpiry::record_the_points($refuserid, $insertpoints,$pointsredeemed,$date,'MREPFU',$equearnamt,$equredeemamt,$order_id,$productid,'0','0','',$totalpoints,'','0');                       
                                }                                
                            }else{
                                RSPointExpiry::insert_earning_points($refuserid,'0','0',$date,'MREPFU',$order_id,'0','0','');
                                $totalpoints = RSPointExpiry::get_sum_of_total_earned_points($refuserid);
