...
class RSFunctionForMyAccount {
    ...
    public function __construct() {
        ...
        add_action('wp_head', array($this, 'add_script_to_dashboard'));
        ...
        add_action( 'pre_get_posts',array($this, 'rs_show_referrer_name_in_home_page')  );
        }
    ...
    public static function count_statistics_referral() {
        ...
        if (isset($_GET['ref'])) {
            if (get_option('rs_referral_cookies_expiry') == '1') {
                ...
                setcookie('rsreferredusername', $_GET['ref'], time() + $min, '/');
    ...
    public static function view_cash_back_table_in_myaccount() {
        ...
        if (!empty($fetcharray)) {
            echo "<h2>" . get_option('rs_my_cashback_title') . "</h2>";
            ?>
    ...
    public static function view_list_table_in_myaccount() {
        ...
        if ($totaloldpoints != '') {
            echo "<h4> " . get_option('rs_my_rewards_total') . " " . round(number_format((float) $totaloldpoints, 2, '.', ''), $roundofftype) . "</h4><br>";
        } else {
        ...
        <table class = "examples demo shop_table my_account_orders table-bordered" data-filter = "#filters" data-page-size="5" data-page-previous-text = "prev" data-filter-text-only = "true" data-page-next-text = "next">
            ...
            <thead><tr><th data-toggle="true" data-sort-initial = "true"><?php echo get_option('rs_my_rewards_sno_label'); ?></th>
                    ...
                    <th><?php echo get_option('rs_my_rewards_points_expired_label'); ?></th>
                        ...
                        if (is_array($newarray)) {
                            if (isset($newarray['earnedpoints'])) {
                                ...
                                if (!empty($newarray['checkpoints'])) {
                                    ...
                                    $pointsexpireddates = $newarray['expirydate'] != 999999999999 ? date('Y-m-d h:i:s', $newarray['expirydate']) : '-';
                                } else {
                                    $rewarderforfrontend = '';
                                }
                            } else {
                            ...
                            if((($pointsearned != 0) && ($redeemedpoints != 0)) || ((($pointsearned != 0) && ($redeemedpoints == 0)) || ($pointsearned == 0) && ($redeemedpoints != 0))){
                            ?>
                            <tr>
                                <td data-value="<?php echo $i; ?>"><?php echo $i; ?></td>
                                <td><?php echo $usernickname; ?> </td>
                                ...
                                <td><?php echo $newarray['earneddate']; ?></td>
                            </tr>
                            <?php
                            }
                            $i++;
    ...
    public static function rs_show_referrer_name_in_home_page($query){
        ...
        if(isset($_GET['ref'])){
            ...
            <h4><?php _e(do_shortcode(get_option('rs_show_hide_generate_referral_message_text')));?></h4> 
            </div>
            <style>
